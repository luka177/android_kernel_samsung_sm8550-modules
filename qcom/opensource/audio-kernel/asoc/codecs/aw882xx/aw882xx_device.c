...
static int aw_dev_reg_fw_update(struct aw_device *aw_dev)
{
	...
	if (aw_dev->force_boost_8v_mode)
		aw_dev->ops.aw_set_force_boost_8v(aw_dev, aw_dev->force_boost_8v_mode);
...
static int aw_dev_mode1_pll_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0)
		...
		aw_dev_info(aw_dev->dev, "pll&clk check done");
...
void aw882xx_dev_init_check(struct aw_device *aw_dev)
{
	struct aw_init_check_desc *init_check_desc = &aw_dev->init_check_desc;

	if (init_check_desc->reg) {
		usleep_range(AW_2000_US, AW_2000_US +10);
		aw_dev->ops.aw_i2c_write(aw_dev, init_check_desc->reg,
						init_check_desc->check_val);
...
int aw882xx_dev_reg_update(struct aw_device *aw_dev, bool force)
{
	...
	if (force || aw_dev->reinit_flag == true) {
		aw882xx_dev_soft_reset(aw_dev);
		ret = aw_dev_reg_fw_update(aw_dev);
		if (ret < 0)
			return ret;

		aw_dev->reinit_flag = false;
	} else {
...
int aw882xx_device_stop(struct aw_device *aw_dev)
{
	...
	if (aw_dev->force_boost_8v_mode != AW_FORCE_BOOST_8V_DISABLE) {
		aw_dev->ops.aw_set_force_boost_8v(aw_dev, AW_FORCE_BOOST_8V_DISABLE);
		aw_dev->force_boost_8v_mode = AW_FORCE_BOOST_8V_DISABLE;
	}
