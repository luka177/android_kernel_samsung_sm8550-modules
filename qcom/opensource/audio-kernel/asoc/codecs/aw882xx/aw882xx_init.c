...
static int aw_pid_2055a_set_bst_mode(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	unsigned int reg_value = 0;

	aw882xx_i2c_read(aw882xx, AW_PID_2055A_BSTCTRL1_REG, &reg_value);
	aw_dev_dbg(aw882xx->dev, "0x%x = 0x%x", AW_PID_2055A_BSTCTRL1_REG, reg_value);
	reg_value = reg_value & AW_PID_2055A_BST_MODE_MASK;

	if (mode_flag) {
		/*0x60[7:6], set BST_MODE: Force Boost*/
		reg_value = reg_value | AW_PID_2055A_BST_MODE_FORCE_BOOST_VALUE;
	} else {
		reg_value = reg_value | AW_PID_2055A_BST_MODE_SMART_BOOST2_VALUE;
	}

	ret = aw882xx_i2c_write(aw882xx, AW_PID_2055A_BSTCTRL1_REG, reg_value);
	if (ret < 0) {
...
static int aw_pid_2055a_set_bst_vout(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	unsigned int reg_value = 0;

	aw882xx_i2c_read(aw882xx, AW_PID_2055A_BSTCTRL2_REG, &reg_value);
	aw_dev_dbg(aw882xx->dev, "0x%x = 0x%x", AW_PID_2055A_BSTCTRL2_REG, reg_value);
	reg_value = reg_value & AW_PID_2055A_VOUT_VREFSET_MASK;

	if (mode_flag) {
		/*0x61[6:0], set VOUT_VREFSET: 8.00 V*/
		reg_value = reg_value | AW_PID_2055A_VOUT_VREFSET_8V_VALUE;
	} else {
		reg_value = reg_value | AW_PID_2055A_VOUT_VREFSET_10P25V_VALUE;
	}

	ret = aw882xx_i2c_write(aw882xx, AW_PID_2055A_BSTCTRL2_REG, reg_value);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c write error, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055a_set_mpd(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	unsigned int reg_value = 0;

	aw882xx_i2c_read(aw882xx, AW_PID_2055A_DACCFG7_REG, &reg_value);
	aw_dev_dbg(aw882xx->dev, "0x%x = 0x%x", AW_PID_2055A_DACCFG7_REG, reg_value);
	reg_value = reg_value & AW_PID_2055A_EN_MPD_MASK;

	if (mode_flag) {
		/*0x0E[6], set EN_MPD: Disable*/
		reg_value = reg_value | AW_PID_2055A_EN_MPD_DISABLE_VALUE;
	} else {
		reg_value = reg_value | AW_PID_2055A_EN_MPD_ENABLE_VALUE;
	}

	ret = aw882xx_i2c_write(aw882xx, AW_PID_2055A_DACCFG7_REG, reg_value);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c write error, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055a_set_force_boost_8v_reg(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;

	aw_dev_dbg(aw882xx->dev, "set force boost 8v, mode=%u", mode_flag);

	ret = aw_pid_2055a_set_bst_mode(aw_dev, mode_flag);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set bst mod failed, ret = %d", ret);
		return ret;
	}

	ret = aw_pid_2055a_set_bst_vout(aw_dev, mode_flag);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set bst vout failed, ret = %d", ret);
		return ret;
	}

	ret = aw_pid_2055a_set_mpd(aw_dev, mode_flag);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set mpd failed, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055a_dev_init(struct aw882xx *aw882xx, int index)
{
	...
	aw_pa->ops.aw_set_force_boost_8v = aw_pid_2055a_set_force_boost_8v_reg;
	...
	aw_pa->init_check_desc.reg = AW_PID_2055A_INIT_CHECK_REG;
	aw_pa->init_check_desc.check_val = AW_PID_2055A_INIT_CHECK_VALUE;
...
static int aw_pid_2055_set_bst_mode(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	unsigned int reg_value = 0;

	aw882xx_i2c_read(aw882xx, AW_PID_2055_BSTCTRL1_REG, &reg_value);
	aw_dev_dbg(aw882xx->dev, "0x%x = 0x%x", AW_PID_2055_BSTCTRL1_REG, reg_value);
	reg_value = reg_value & AW_PID_2055_BST_MODE_MASK;

	if (mode_flag) {
		/*0x60[7:6], set BST_MODE: Force Boost*/
		reg_value = reg_value | AW_PID_2055_BST_MODE_FORCE_BOOST_VALUE;
	} else {
		reg_value = reg_value | AW_PID_2055_BST_MODE_CLASS_H_VALUE;
	}

	ret = aw882xx_i2c_write(aw882xx, AW_PID_2055_BSTCTRL1_REG, reg_value);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c write error, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055_set_bst_vout(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	unsigned int reg_value = 0;

	aw882xx_i2c_read(aw882xx, AW_PID_2055_BSTCTRL2_REG, &reg_value);
	aw_dev_dbg(aw882xx->dev, "0x%x = 0x%x", AW_PID_2055_BSTCTRL2_REG, reg_value);
	reg_value = reg_value & AW_PID_2055_BST_VOUT_SET_MASK;

	if (mode_flag) {
		/*0x61[6:0], set BST_VOUT_SET: 8.00 V*/
		reg_value = reg_value | AW_PID_2055_BST_VOUT_SET_8V_VALUE;
	} else {
		reg_value = reg_value | AW_PID_2055_BST_VOUT_SET_10P25V_VALUE;
	}

	ret = aw882xx_i2c_write(aw882xx, AW_PID_2055_BSTCTRL2_REG, reg_value);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c write error, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055_set_mpd(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	unsigned int reg_value = 0;

	aw882xx_i2c_read(aw882xx, AW_PID_2055_DACCFG5_REG, &reg_value);
	aw_dev_dbg(aw882xx->dev, "0x%x = 0x%x", AW_PID_2055_BSTCTRL2_REG, reg_value);
	reg_value = reg_value & AW_PID_2055_EN_MPD_MASK;

	if (mode_flag) {
		/*0x0D[15], set EN_MPD: Disable*/
		reg_value = reg_value | AW_PID_2055_EN_MPD_DISABLE_VALUE;
	} else {
		reg_value = reg_value | AW_PID_2055_EN_MPD_ENABLE_VALUE;
	}

	ret = aw882xx_i2c_write(aw882xx, AW_PID_2055_DACCFG5_REG, reg_value);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "i2c write error, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055_set_force_boost_8v_reg(struct aw_device *aw_dev, unsigned int mode_flag)
{
	int ret = -EINVAL;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	...
	ret = aw_pid_2055_set_bst_mode(aw_dev, mode_flag);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set bst mod failed, ret = %d", ret);
		return ret;
	}

	ret = aw_pid_2055_set_bst_vout(aw_dev, mode_flag);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set bst vout failed, ret = %d", ret);
		return ret;
	}

	ret = aw_pid_2055_set_mpd(aw_dev, mode_flag);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set mpd failed, ret = %d", ret);
		return ret;
	}
...
static int aw_pid_2055_dev_init(struct aw882xx *aw882xx, int index)
{
	...
	aw_pa->ops.aw_set_force_boost_8v = aw_pid_2055_set_force_boost_8v_reg;
	...
	aw_pa->volume_desc.max_volume = AW_PID_2055_VOL_DEFAULT_VALUE;
	...
	aw_pa->init_check_desc.reg = AW_PID_2055_INIT_CHECK_REG;
	aw_pa->init_check_desc.check_val = AW_PID_2055_INIT_CHECK_VALUE;
...
static int aw_pid_2113_dev_init(struct aw882xx *aw882xx, int index)
{
	...
	aw_pa->init_check_desc.reg = AW_PID_2113_INIT_CHECK_REG;
	aw_pa->init_check_desc.check_val = AW_PID_2113_INIT_CHECK_VALUE;
