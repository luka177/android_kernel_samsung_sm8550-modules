	...
	int fs_previous_profile)
{
	...
	/* ---------- a/default settings previous profile ---------- a/*/
	...
	int prof_idx, int vstep_idx)
{
	...
	if (tfa->verbose)
		pr_debug("---------- a/default settings profile: %s (%d) ----------\n",
	...
	if (tfa->verbose)
		pr_debug("---------- a/new settings profile: %s (%d) ----------\n",
			tfa_cont_get_string(tfa->cnt,
			&prof->name), prof_idx);
	...
	if ((tfa->ext_dsp != 0) && (tfa->tfa_family == 2)) {
		for (i = 0; i < previous_prof_tfadsp->length; i++) {
			...
			if (tfa->verbose && k == 0) {
				pr_debug("---------- a/files default profile: %s (%d) ----------\n",
					tfa_cont_get_string
		...
		if (tfa->verbose)
			pr_debug("---------- a/files new profile: %s (%d) ----------\n",
				tfa_cont_get_string(tfa->cnt,
