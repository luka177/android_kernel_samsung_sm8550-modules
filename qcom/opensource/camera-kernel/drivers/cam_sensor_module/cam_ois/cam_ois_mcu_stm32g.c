...
#define OIS_DM1Q_DM2Q_FIRST_VERSION    "YBC1PE1A"
#define OIS_DM3Q_FIRST_VERSION         "ZGC1PG1A"
...
int target_empty_check_status(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	/* Read first flash memory word ------------------------------------------- a/*/
...
int target_empty_check_clear(struct cam_ois_ctrl_t * o_ctrl)
{
	...
	/* Option byte write (dummy: readed value) -------------------------------- a/*/
	ret = sysboot_i2c_write(o_ctrl, memory_map.optionbyte, (uint8_t *)&optionbyte, sizeof(optionbyte));
	if (ret < 0) {
	...
	msleep(150);
	/* Option byte read for checking protection status ------------------------ a/*/
